...
(defun mt-terminus-font (bold)
  ...
  (let ((weight (if bold "bold" "medium")))
    (set-default-font
     (format "-xos4-terminus-%s-*-*-*-12-*-*-*-*-*-*-*" weight))))
...
(defun mt-insert-signature ()
  (interactive)
  (insert (shell-command-to-string "~/bin/mksig")))
...
(defun mt-insert-excuse (prefix)
  "Insert what we're playing right now."
  (interactive "P")
  (save-excursion
    (let ((coding-system-for-read 'utf-8))
      (call-process "/Users/mit/bin/excuse.rb" nil t nil))
    (backward-delete-char 1))
  (goto-char (point-at-eol)))
