...
;; Time-stamp: <2007-07-17 18:55:52 csh>
...
;; buffer object alive.  If the actual content is important to keep in
;; a config, consider adding these major modes to the
;; `policy-switch-live-buffer-modes-restore' variable.  Then, they
;; will _always_ be restored when a config is selected (the drawback
;; is increased overhead)
...
must be create is current in the call.  Functions take no
...
(defun policy-switch-window-info ()
  "Get window data from current window configuration."
  (let ((window-data ()))
    (dolist (buffer-data (winner-win-data))
      (let* ((buffer-obj (cdr buffer-data)))
	(setcdr buffer-data (list buffer-obj (policy-switch-buffer-info-string buffer-obj) (buffer-name buffer-obj)))
